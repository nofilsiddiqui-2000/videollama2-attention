...
def calculate_ultimate_metrics(img1, img2):
    """Calculate comprehensive attack metrics with temporal awareness"""
    mse = torch.mean((img1 - img2) ** 2)
    psnr = 20 * torch.log10(2.0 / torch.sqrt(mse)) if mse > 0 else float('inf')
    linf_norm = torch.max(torch.abs(img1 - img2)).item()
    l2_norm = torch.norm(img1 - img2).item()
    
    # GPT ENHANCEMENT 6: Per-frame metrics (temporal PSNR)
    frame_psnrs = []
    for t in range(img1.shape[0]):
        frame_mse = torch.mean((img1[t] - img2[t]) ** 2)
        frame_psnr = 20 * torch.log10(2.0 / torch.sqrt(frame_mse)) if frame_mse > 0 else float('inf')
        frame_psnrs.append(frame_psnr.item())
    
    min_frame_psnr = min(frame_psnrs)  # Strictest quality control
    ...
    def ssim_enhanced(x, y):
        ...
        c1, c2 = (0.01 * 2)**2, (0.03 * 2)**2  # Scaled for [-1,1] range
    ...
    return psnr.item(), linf_norm, l2_norm, ssim, min_frame_psnr

def semantic_similarity_ultimate(text1: str, text2: str) -> float:
    """Ultimate semantic similarity with CLIP scoring"""
    try:
        # Try CLIP first for better alignment with vision models
        try:
            from transformers import CLIPProcessor, CLIPModel
            clip_model = CLIPModel.from_pretrained("openai/clip-vit-base-patch32")
            clip_processor = CLIPProcessor.from_pretrained("openai/clip-vit-base-patch32")
            
            inputs = clip_processor(text=[text1, text2], return_tensors="pt", padding=True)
            text_features = clip_model.get_text_features(**inputs)
            similarity = F.cosine_similarity(text_features[0:1], text_features[1:2], dim=1).item()
            return similarity
        except:
            pass
...
# GPT ENHANCEMENT 2: Advanced input transformations
def ultimate_di_transform(x, p=0.8):
    ...
    if torch.rand(1).item() < p:
        ...
        # GPT ENHANCEMENT 2: Add Gaussian noise
        if torch.rand(1).item() < 0.5:
            x_padded = x_padded + torch.randn_like(x_padded) * 0.03
        
        # GPT ENHANCEMENT 2: Random channel shuffle
        if torch.rand(1).item() < 0.3:
            perm = torch.randperm(3)
            x_padded = x_padded[:, perm]
        
        # GPT ENHANCEMENT 2: Spatial jitter
        if torch.rand(1).item() < 0.5:
...
def ultimate_ti_transform_grad(grad, kernel_size=9, sigma=2.0):
    ...
    # Larger kernel for stronger smoothing
...
def ultimate_low_frequency_perturbation(grad, keep_ratio=0.05):
    """GPT ENHANCEMENT 2: Ultra low-frequency perturbation for imperceptibility"""
...
# GPT ENHANCEMENT 1.2: Scale-Invariant Multi-scale gradients
def sim_multi_scale_grad(vlm, adv_tensor, scales=[1.0, 0.75, 0.5], compute_loss_fn=None):
    """Scale-Invariant Multi-scale gradient computation"""
    sim_grad = torch.zeros_like(adv_tensor)
    
    for scale in scales:
        if scale != 1.0:
            # Resize to scale
            h, w = adv_tensor.shape[-2:]
            new_h, new_w = int(h * scale), int(w * scale)
            resized = F.interpolate(adv_tensor, size=(new_h, new_w), 
                                  mode='bilinear', align_corners=False)
            
            # Resize back to original size for gradient
            resized_back = F.interpolate(resized, size=(h, w), 
                                       mode='bilinear', align_corners=False)
            resized_back = resized_back.requires_grad_(True)
        else:
            resized_back = adv_tensor
        
        # Forward pass
        features = vlm.model.vision_tower(resized_back)
        loss = compute_loss_fn(features) if compute_loss_fn else -(features.pow(2).mean())
        
        # Get gradients
        grad = torch.autograd.grad(loss, resized_back, create_graph=False)[0]
        sim_grad += grad / len(scales)
...
def load_models(device="cuda", verbose=True):
    ...
    vlm, vprocessor, tok = model_init(
        ...
        torch_dtype=torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16,  # GPT ENHANCEMENT 5
        device_map="auto",
...
class UltimateAttack:
    ...
    def __init__(self, vlm, tokenizer, device="cuda", verbose=True):
        ...
        self.target_cache = {}  # Cache for targeted loss computation
    
    def compute_ultimate_loss(self, features, video_tensor=None, target_text=None, weights=[0.8, 0.2]):
        """Ultimate multi-objective loss with caching"""
        ...
        # Cached targeted component
        if target_text and video_tensor is not None:
            cache_key = f"{target_text}_{video_tensor.shape}"
            ...
            targeted_loss = torch.tensor(self.target_cache[cache_key], device=self.device)
    ...
    def ultimate_ni_mi_fgsm(self, video_tensor, epsilon=0.07, num_steps=15, alpha=None, 
        ...
        """
        GPT ENHANCEMENT 1.1: Nesterov + Momentum FGSM (NI-MI-FGSM)
        GPT ENHANCEMENT 1.2: Scale-Invariant Multi-scale
        GPT ENHANCEMENT 2: All input transforms
        GPT ENHANCEMENT 5: Memory optimizations
        """
        if alpha is None:
        ...
        # GPT ENHANCEMENT: Initialize momentum
        ...
        # GPT ENHANCEMENT 3: Temporal momentum for video
        temporal_momentum = torch.zeros_like(adv_tensor)
        ...
        for step in range(num_steps):
            ...
            try:
                # GPT ENHANCEMENT 5: Memory management
                if step % 3 == 0:  # Clear cache every 3 steps
                    clear_memory()
                
                # Create fresh tensor with gradients
                adv_tensor = adv_tensor.detach().requires_grad_(True)
                
                # GPT ENHANCEMENT 2: Ultimate input transforms
                transformed_tensor = ultimate_di_transform(adv_tensor, p=0.8)
                
                # GPT ENHANCEMENT 1.2: Multi-scale gradient computation
                try:
                    # Try multi-scale if memory allows
                    grad = sim_multi_scale_grad(
                        self.vlm, 
                        transformed_tensor, 
                        scales=[1.0, 0.75, 0.5],
                        compute_loss_fn=lambda feats: self.compute_ultimate_loss(feats, adv_tensor, target_text)
                    )
                    ...
                    if "memory" in str(e).lower():
                        ...
                        loss.backward(create_graph=False)  # GPT ENHANCEMENT 5
                        grad = adv_tensor.grad.detach()
                    else:
                        raise e
                
                if grad is not None:
                    # GPT ENHANCEMENT 2: Apply transforms to gradients
                    ...
                    # GPT ENHANCEMENT 3: Temporal momentum
                    temporal_momentum = 0.8 * temporal_momentum + 0.2 * normalized_grad
                    
                    # GPT ENHANCEMENT 1.1: Nesterov momentum update
                    ...
                    with torch.no_grad():
                        ...
                        # Track best
                        current_loss = self.compute_ultimate_loss(
                            self.vlm.model.vision_tower(adv_tensor.detach()), 
                            adv_tensor, target_text
                        ).item()
                        ...
                        if current_loss < best_loss:
                            ...
                            best_adv = adv_tensor.clone()
                            if self.verbose:
                                print(f"     ✨ New best: {current_loss:.6f}")
...
def ultimate_attack_pipeline(video_path, vlm, vprocessor, tok, 
                           attack_type="ultimate", epsilon=0.07, 
                           num_steps=15, target_text=None, 
    ...
    # Smart frame sampling with key frame detection
    target_frames = 4
    ...
    perceptual_quality = min(psnr, min_frame_psnr)  # Strictest quality measure
    
    # Ultimate success criteria (much stricter)
